...
async function test() {
  ...
  const updateQuestion = questionList.filter( q => q.title === question.title )[ 0 ];
  updateQuestion.title = 'An Updated Title';
  updateQuestion.text = 'A whole new world!';
  await CanvasPuppet.question.update( page, rootUrl, courseList[ courseId ], questionBankList[ qbId ], updateQuestion );
